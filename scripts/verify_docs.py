...
def check_file(filepath: Path, expected_tools: int, expected_papers: int, expected_skills: int):
    ...
    # Check tool count claims (common patterns: "43 工具", "43 个工具", "43 tools")
    for m in re.finditer(r'(\d+)\s*(?:个)?(?:工具|tools)', content, re.IGNORECASE):
        claimed = int(m.group(1))
        if claimed != expected_tools and claimed >= 30:  # heuristic: only check tool-related numbers
            errors.append(f"  {rel}: claims {claimed} tools, actual is {expected_tools}")

    # Check paper count claims (patterns: "445+ 篇", "560+ 篇", "563 篇")
    for m in re.finditer(r'(\d+)\+?\s*篇', content):
        claimed = int(m.group(1))
        if claimed < expected_papers and claimed >= 100:  # heuristic: only check large counts
