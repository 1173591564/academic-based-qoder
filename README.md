...
scholar stats                             # 验证
scholar init-workspace ~/any-project      # 一键配置到任意项目
scholar doctor                            # 验证配置
```

`init-workspace` 一条命令自动完成：
- 创建 `output/drafts/`、`output/notes/`、`output/logs/`
- 复制 `.qoder/`、`.claude/` 配置（rules、skills、hooks）
- 生成正确的 `mcp.json`（`SCHOLAR_HOME` 指向论文数据，`SCHOLAR_WORKSPACE` 指向目标项目）
...
python -m scholar init-workspace [target]   # 一键配置到任意项目
```
...
`infra/docker-compose.yml` 已配置 `restart: unless-stopped`，Docker Desktop 启动后容器会自动拉起。
如果未生效，检查 Docker Desktop 设置 → General → Start Docker Desktop when you sign in。
