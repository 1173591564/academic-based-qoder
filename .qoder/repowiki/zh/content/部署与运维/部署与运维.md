...
## 项目结构
该项目采用“前端插件 + 后端 Python 应用 + 容器化基础设施”的分层架构。核心目录与职责如下：
- infra：容器编排与数据库初始化脚本
...
## 架构总览
下图展示容器化部署的整体交互：CLI 通过配置模块读取环境变量，连接 PostgreSQL 与 Neo4j；容器内数据库由 init.sql 初始化；启动脚本负责编排与健康检查。
...
## 结论
本指南提供了从容器编排、数据库初始化、环境变量管理到服务健康检查与运维实践的完整方案。通过一键启动脚本与清晰的配置体系，可以快速搭建并稳定运行学术研究工具链。建议在生产环境中结合监控告警、备份恢复与安全加固进一步提升可靠性与安全性。
