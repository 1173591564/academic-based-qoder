...
**更新** 写作工作流已从传统的线性处理模式转变为迭代处理模式，引入了 Draft-Revise-Finalize 循环和质量门控机制，支持多轮审阅、修改和最终定稿的完整生命周期管理。
...
Drafts["草稿/修订版本<br/>output/drafts/"]
...
- **迭代写作管道**：Draft-Revise-Finalize 循环、质量门控、多轮审阅管理
- CLI 与 MCP：统一命令行接口、与 Qoder 的桥接层

**更新** 新增迭代写作管道组件，支持草稿生成、修订管理和最终定稿的完整工作流。

**章节来源**
- [scholar/cli.py: 1-800:1-800](file://scholar/cli.py#L1-L800)
- [scholar/config.py: 1-50:1-50](file://scholar/config.py#L1-L50)
...
**更新** 新增迭代写作管道，支持草稿生成、修订管理和最终定稿的循环处理模式。
...
participant Draft as "草稿生成器"
participant Review as "质量门控"
...
### 迭代写作管道（新增）
- 草稿生成：基于解析后的结构化数据生成初稿草稿
- 修订管理：支持多轮修订，每次修订都有明确的质量门控检查
- 质量门控：7维质量评分作为修订决策依据
- 最终定稿：通过质量门控后生成最终版本
- 版本管理：草稿目录（output/drafts/）支持多版本管理

```mermaid
flowchart TD
Start(["开始写作流程"]) --> Draft["生成初稿草稿"]
Draft --> Quality["质量评估"]
Quality --> Gate{"质量门控通过？"}
Gate --> |否| Revise["根据质量报告进行修订"]
Revise --> Quality
Gate --> |是| Finalize["生成最终定稿"]
Finalize --> Complete(["完成"])
```

**图表来源**
- [scholar/config.py: 25-35:25-35](file://scholar/config.py#L25-L35)
- [scholar_mcp/server.py: 300-320:300-320](file://scholar_mcp/server.py#L300-L320)
- [scholar_mcp/server.py: 540-560:540-560](file://scholar_mcp/server.py#L540-L560)

**章节来源**
- [scholar/config.py: 1-50:1-50](file://scholar/config.py#L1-L50)
- [scholar_mcp/server.py: 1-600:1-600](file://scholar_mcp/server.py#L1-L600)
...
**更新** 新的迭代写作管道显著提升了论文写作的质量控制和效率，通过 Draft-Revise-Finalize 循环和质量门控机制，确保每篇论文都能经过充分的审阅和修订，最终产出高质量的学术成果。
...
- **新增** 迭代写作管道
  - 草稿生成：python -m scholar generate-draft <ULID>
  - 质量评估：python -m scholar quality-score <ULID>
  - 修订管理：python -m scholar revise-draft <ULID>
  - 最终定稿：python -m scholar finalize-paper <ULID>
- 外部集成
