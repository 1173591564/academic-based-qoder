...
- [test/test_db_pool.py](file://test/test_db_pool.py)
- [scholar/graph_db.py](file://scholar/graph_db.py)
</cite>

## 更新摘要
**变更内容**
- 改进的数据库连接生命周期管理：增强cursor上下文管理器的异常处理和finally块的连接池管理
- 连接池优化：修复连接池泄漏问题，确保连接在异常情况下也能正确归还
- 新增连接池单元测试：添加7个测试验证pool模式实现，包括连接借用/归还平衡、异常处理和可用性检测
- 连接池资源管理改进：异常处理增强，确保连接正确归还，优雅降级机制
...
**更新** 本版本新增了连接池架构支持，Database类现在支持可选的连接池参数，实现高效的连接复用和资源管理，特别适用于MCP服务器的长期运行场景。**重点修复**了连接池模式下的资源泄漏问题，确保异常情况下的连接正确归还。**新增**了完整的连接池单元测试套件，验证pool模式的正确性和稳定性。
...
- 单元测试：验证文件回退与可用性检测逻辑，**新增**连接池模式的完整测试套件
...
TEST["单元测试套件<br/>连接池测试<br/>文件回退测试"]
end
subgraph "外部系统"
...
- [test/test_db_pool.py:1-137](file://test/test_db_pool.py#L1-137)
...
- 单元测试：验证文件回退与可用性检测，**新增**连接池模式的完整测试套件
- 图数据库集成：Neo4j 引入概念图谱与引用网络，作为补充存储

**更新** 新增连接池支持，Database类现在可以接受外部连接池实例，实现高效的连接复用和资源管理。**重点修复**了连接池模式下的异常处理，确保连接泄漏问题得到解决。**新增**了7个连接池单元测试，覆盖连接借用/归还平衡、异常处理和可用性检测。
...
- [test/test_db_pool.py:66-137](file://test/test_db_pool.py#L66-L137)

## 架构总览
数据库抽象层采用"可选 PostgreSQL + 连接池 + 文件系统回退"的三层设计。Database 类通过上下文管理器确保事务的提交或回滚；当 PostgreSQL 可用时，优先使用数据库；连接池模式下，MCP服务器可以高效复用连接；否则回退到文件系统进行读写。**新增**的测试套件确保了连接池模式的稳定性和可靠性。
...
- [test/test_db_pool.py:81-122](file://test/test_db_pool.py#L81-L122)
...
- 事务处理
  ...
  - **异常处理增强**：连接池模式下确保putconn在异常情况下也能执行

**更新** Database类现在支持连接池模式，构造函数新增pool参数，cursor上下文管理器根据是否存在连接池选择不同的连接借用/归还策略。**重点修复**了连接池模式下的异常处理，确保即使在异常情况下也能正确归还连接，防止连接池泄漏。**新增**了完整的单元测试验证这些行为。
...
+test_getconn_called_on_enter()
+test_putconn_called_on_exit()
+borrow_return_balance()
+test_putconn_called_on_exception()
+test_pool_not_used_when_none()
}
class TestDatabaseAvailability {
+test_available_with_pool()
}
Database --> SharedState : "可选依赖"
TestDatabasePoolMode --> Database : "验证连接池行为"
TestDatabaseAvailability --> Database : "验证可用性检测"
```
...
**新增** SharedState类是连接池架构的核心组件，负责进程级的连接池生命周期管理。**新增**的测试套件验证了其初始化和清理功能的正确性。

**章节来源**
- [scholar/_state.py:20-131](file://scholar/_state.py#L20-L131)
- [test/test_db_pool.py:95-104](file://test/test_db_pool.py#L95-L104)

### 连接池单元测试套件
**新增** 完整的连接池测试套件包含7个测试，验证Database.cursor()中的pool模式实现：

- **连接借用测试**：验证cursor()进入上下文时调用pool.getconn()
- **连接归还测试**：验证cursor()退出上下文时调用pool.putconn()
- **借用归还平衡测试**：验证每次getconn都有对应的putconn
- **异常处理测试**：验证异常情况下putconn仍会被调用
- **无池模式测试**：验证当_pool为None时使用单连接路径
- **可用性检测测试**：验证available属性使用pool.getconn/putconn测试

**测试特点**：
- 使用FakePool、FakeConnection、FakeCursor模拟组件
- 确保getconn()/putconn()在异常场景下正确平衡
- 验证事务错误处理和pool-unavailable降级条件
- 测试连接生命周期管理

**章节来源**
- [test/test_db_pool.py:11-137](file://test/test_db_pool.py#L11-L137)
...
- **新增** 文件回退测试：验证save_parsed、load_parsed、list_parsed的正确性
...
- 内部依赖
  ...
  - 文件回退函数依赖配置模块提供的输出目录
  - **新增** 测试套件依赖Database类和SharedState类
- 外部依赖
...
**更新** 新增SharedState类对Database类的依赖，以及对ThreadedConnectionPool的可选依赖。**新增**测试套件对Database类和SharedState类的依赖关系。
...
- [test/test_db_pool.py:76-78](file://test/test_db_pool.py#L76-L78)
...
- [test/test_db_pool.py:72-79](file://test/test_db_pool.py#L72-L79)
...
- **新增** 连接池问题
  ...
  - **新增** 测试验证：使用单元测试套件验证连接池行为
- **新增** 测试套件问题
  - 现象：连接池测试失败
  - 排查：检查FakePool、FakeConnection、FakeCursor的模拟实现
  - 处理：确保模拟组件正确实现psycopg2接口
...
## 结论
该数据库抽象层通过"可选 PostgreSQL + 连接池 + 文件系统回退"实现了高可用与易用性的平衡：在具备数据库时提供强一致与高性能查询，在不具备数据库时仍可稳定运行。**新增的连接池架构**特别适用于MCP服务器的长期运行场景，通过高效的连接复用显著提升了并发性能。**重点修复**的连接池泄漏问题确保了系统的稳定性，即使在异常情况下也能正确管理连接资源。**新增的完整测试套件**进一步增强了系统的可靠性，验证了连接池模式下的各种边界情况和异常处理。结合合理的索引与批量操作策略，可在中小规模场景下获得良好体验；对于更大规模与更复杂的查询需求，建议引入连接池、全文检索与向量化扩展（如 pgvector）以进一步提升性能与功能。
...
- **新增** 连接池测试：使用单元测试套件验证连接池行为的正确性

**新增** 连接池架构为MCP服务器提供了稳定的数据库访问能力，支持高并发场景下的高效连接管理。**重点修复**的异常处理机制确保了系统的可靠性。**新增**的测试套件进一步保证了连接池功能的稳定性。
...
- **测试验证**：使用FakePool、FakeConnection、FakeCursor模拟组件验证连接池行为
...
### 连接池单元测试详细说明
**新增** 完整的连接池测试套件包含以下测试用例：

1. **连接借用测试** (`test_getconn_called_on_enter`)
   - 验证cursor()进入上下文时调用pool.getconn()
   - 使用断言验证borrowed列表长度为1

2. **连接归还测试** (`test_putconn_called_on_exit`)
   - 验证cursor()退出上下文时调用pool.putconn()
   - 使用断言验证returned列表长度为1

3. **借用归还平衡测试** (`test_borrow_return_balance`)
   - 验证每次getconn都有对应的putconn()
   - 测试多个上下文的平衡性

4. **异常处理测试** (`test_putconn_called_on_exception`)
   - 验证异常情况下putconn仍会被调用
   - 确保连接不会泄漏

5. **无池模式测试** (`test_pool_not_used_when_none`)
   - 验证当_pool为None时使用单连接路径
   - 防止尝试使用不存在的池

6. **可用性检测测试** (`test_available_with_pool`)
   - 验证available属性使用pool.getconn/putconn测试
   - 确保连接池可用性检测正确

7. **模拟组件测试**
   - FakePool：模拟psycopg2 ThreadedConnectionPool接口
   - FakeConnection：模拟psycopg2连接对象
   - FakeCursor：模拟psycopg2游标对象

**测试特点**：
- 使用unittest.mock创建模拟组件
- 验证连接池生命周期管理
- 测试异常场景下的资源清理
- 确保事务错误处理的正确性
...
**章节来源**
- [test/test_db_pool.py:66-137](file://test/test_db_pool.py#L66-L137)
- [test/test_db_pool.py:11-56](file://test/test_db_pool.py#L11-L56)

### 改进的连接生命周期管理
**更新** 数据库连接生命周期管理得到了显著改进：

- **增强的异常处理**：在cursor上下文管理器中，finally块确保即使发生异常也会正确归还连接
- **连接池泄漏防护**：通过try-except确保putconn在异常情况下也能执行，防止连接池泄漏
- **优雅降级机制**：当连接池出现异常时，系统会自动降级到文件回退模式
- **资源清理优化**：SharedState.close()方法确保连接池完全关闭，避免资源泄露
- **测试覆盖增强**：使用FakePool、FakeConnection、FakeCursor模拟组件验证连接池行为的正确性
