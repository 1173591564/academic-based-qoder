...
## 简介
本文件聚焦于 Scholar Studio 的 CLI 核心命令，系统性地梳理 scan、parse、parse-all、info、search、list-papers 等基础命令的功能、参数、输入输出、错误处理与性能特征，并给出命令组合、批量处理策略与最佳实践。这些命令围绕“论文目录扫描、TeX 解析、结构化入库、检索与浏览”这一主流程展开，既可独立使用，也可串联形成端到端工作流。

## 项目结构
- CLI 入口通过模块入口调用命令实现，核心命令集中在 scholar/cli.py 中，数据库交互封装在 scholar/db.py。
...
## 架构总览
下图展示了核心命令在系统中的位置与依赖关系：命令层负责参数解析与业务编排，数据层负责持久化与查询，外部资源包括文件系统与可选的 PostgreSQL。
...
图表来源
- [cli.py:46-416](file://scholar/cli.py#L46-L416)
- [db.py:24-200](file://scholar/db.py#L24-L200)
...
```mermaid
graph LR
CLI["命令实现<br/>cli.py"] --> DB["数据库封装<br/>db.py"]
DB --> PG["PostgreSQL(可选)"]
DB --> FS["文件系统(JSON)"]
```
...
## 结论
核心命令围绕“扫描—解析—入库—检索—浏览”构建了完整的论文知识库工作流。通过可选的数据库增强与丰富的参数选项，既能满足日常使用，也能支撑规模化论文管理。建议在生产环境中启用 PostgreSQL，并结合批量与限制策略进行高效运维。

## 附录

### 命令组合与最佳实践
- 建议的工作流
  - 初始化：python -m scholar scan → 检查源码/PDF/解析状态。
  - 批量入库：python -m scholar parse-all --limit N（或 --force）→ 观察进度与失败列表。
  - 质量与浏览：python -m scholar stats → python -m scholar list-papers --year Y --limit L → python -m scholar info <paper_id>。
  - 检索：python -m scholar search "<关键词>" --limit K。
- 批量处理策略
  - 小规模：一次性 parse-all。
