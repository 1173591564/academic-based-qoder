...
## 更新摘要
**变更内容**
- 更新研究循环引擎的简化实现：从39行简化到387行，核心功能保持不变
- 改进错误处理和性能优化
- 增强日志分析功能（支持项目化日志目录）
- 完善原子写入机制和跨关键词去重逻辑
...
**更新** 研究循环引擎经过重构，从简化的39行实现发展为功能完整的387行实现，保持核心功能的同时显著增强了错误处理能力和性能优化。
...
- scholar/commands/research_ops.py：研究循环相关CLI命令实现
- plugin/skills/*：配套技能文档，定义端到端工作流与交互

```mermaid
graph TB
subgraph "研究循环引擎"
RL["research_loop.py<br/>兴趣管理/日志分析/方向同步<br/>387行实现"]
KBU["kb_update.py<br/>arXiv下载/批量入库"]
CFG["config.py<br/>配置/环境变量/arXiv请求"]
CLI["cli.py<br/>命令行入口"]
ROPS["research_ops.py<br/>CLI命令实现"]
end
subgraph "插件技能"
...
**图示来源**
- [research_loop.py:1-387](file://scholar/research_loop.py#L1-L387)
...
- [research_ops.py:1-404](file://scholar/commands/research_ops.py#L1-L404)
...
- 兴趣管理（research_loop.interests）
  ...
  - **增强**：改进的去重逻辑，支持大小写不敏感匹配和更完善的合并策略
- 日志分析（research_loop.logs）
  - 周日志扫描：支持项目化日志目录 output/logs/<project>/week-*.jsonl
  - 解析与标记：逐行 JSONL 解析，完成分析后写入 analyzed.json（原子写入）
  - **增强**：向后兼容旧的平面文件格式，支持多项目日志管理
...
- 知识库更新（kb_update.kb_update）
  ...
  - **增强**：优化的速率限制和更好的失败清理机制
- 配置与环境（config）
  - 目录结构、数据库/图数据库/嵌入配置、arXiv 请求封装（重试/超时/代理）
  - **增强**：改进的环境变量加载和项目化目录支持

**章节来源**
- [research_loop.py:22-313](file://scholar/research_loop.py#L22-L313)
- [kb_update.py:88-473](file://scholar/kb_update.py#L88-L473)
- [config.py:75-237](file://scholar/config.py#L75-L237)
...
participant ROPS as "研究命令"
participant KBU as "知识库更新"
participant DB as "数据库/图谱"
User->>CLI : 触发研究同步单方向/全部
CLI->>ROPS : research-sync 命令
ROPS->>RL : sync_direction/category 或 sync_all_directions
RL->>RL : 加载兴趣画像/解析关键词
...
**图示来源**
- [research_loop.py:209-313](file://scholar/research_loop.py#L209-L313)
...
Decision --> |新增| Dedup["按类别去重合并关键词<br/>大小写不敏感匹配"]
Dedup --> Save["原子写入兴趣画像"]
...
```mermaid
flowchart TD
S(["入口：get_unanalyzed_logs"]) --> Scan["扫描项目化日志目录<br/>output/logs/<project>/week-*.jsonl"]
Scan --> Exists{"是否存在？"}
Exists --> |否| ReturnEmpty["返回空结果"]
Exists --> |是| ReadDone["读取各项目 analyzed.json"]
ReadDone --> Diff["差集 = 未分析周"]
Diff --> Pick["取最早一周"]
Pick --> Parse["逐行解析 JSONL<br/>注入项目字段"]
Parse --> Done(["返回项目化文件路径与条目列表"])
Mark(["入口：mark_week_analyzed"]) --> ReadDone2["读取/更新项目 analyzed.json"]
ReadDone2 --> Upsert["写入/更新记录"]
Upsert --> Atomic["原子写入 .tmp -> 替换"]
```

**图示来源**
- [research_loop.py:102-170](file://scholar/research_loop.py#L102-L170)
...
- [research_loop.py:151-170](file://scholar/research_loop.py#L151-L170)
- [research_loop.py:209-313](file://scholar/research_loop.py#L209-L313)
...
```mermaid
flowchart TD
Log["项目化周日志<br/>output/logs/<project>/week-*.jsonl"] --> IR["兴趣识别/去重<br/>注入项目字段"]
IR --> KW["关键词/类别"]
KW --> Search["arXiv 搜索"]
...
KBU --> CFG
KBU --> DB["数据库/图谱(可选)"]
CLI["cli.py"] --> ROPS["research_ops.py"]
ROPS --> RL
```
...
- [research_ops.py:9-11](file://scholar/commands/research_ops.py#L9-L11)
...
**更新** 经过重构的387行实现版本，在保持核心功能完整性的同时，显著提升了错误处理能力、性能表现和系统稳定性，为复杂研究场景提供了更加可靠的基础设施。
